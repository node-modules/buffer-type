...
var jpegExif = function (buf) {
  ...
  // 数据长度 length	2 bytes	APPn的总长度，不包括marker的２bytes
  ...
  while (offset < buf.length) {
    var flag = buf.slice(offset, offset + 2);
    var size = buf.readUInt16BE(offset + 2);
    // if (flag[0] === 0xff && flag[1] == (0xe0 + appN)) {
      // var content = buf.slice(offset + 4, size)
      // console.log('find app%d, length: %d', appN, size);
      // console.log(content);
    // }
    if (flag[0] === 0xff && flag[1] === 0xc0) {
      sof0 = offset;
...
var types = [gif, png, jpeg, jpegExif, bmp, webp];
